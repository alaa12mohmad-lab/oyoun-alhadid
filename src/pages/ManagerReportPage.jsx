...
import {
  ...
  subWeeks, subMonths, differenceInDays, parseISO
} from 'date-fns'

const STATUS_AR = { working: 'شغالة', breakdown: 'عطل', maintenance: 'صيانة', idle: 'راحة' }
...
export default function ManagerReportPage() {
  ...
  // Date range based on mode + offset
  function getRange() {
    const now = new Date()
    if (mode === 'weekly') {
      ...
      logs.forEach(l => {
        if (!eqSummary[l.equipmentId]) {
          ...
          eqSummary[l.equipmentId] = {
            ...
            hours: 0, cost: 0,
            workDays: 0, breakdownDays: 0, maintenanceDays: 0, idleDays: 0,
            breakdownReasons: [],
      ...
      <div className="page no-print">
        <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            <button className="btn btn-secondary" onClick={shareWhatsapp} disabled={!data}>
              📲 واتساب
            </button>
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 24, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            {['weekly', 'monthly'].map(m => (
              <button key={m} onClick={() => { setMode(m); setOffset(0) }}
                style={{
                  ...
                  color: mode === m ? '#1a1200' : 'var(--text-2)',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div style={{ background: 'var(--steel-3)', border: '1px solid var(--accent)', borderRadius: 8, padding: '7px 18px', fontWeight: 600, fontSize: '0.9rem', color: 'var(--accent)' }}>
              {label}
            </div>
        ...
        {loading ? <div className="spinner" /> : !data ? null : (
          <div className="card">
            <div className="card-header">
              <span className="card-title">معاينة التقرير — {data.label}</span>
            ...
            <div className="card-body">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 12, marginBottom: 24 }}>
                {[
                  ...
                  { val: data.siteList.length,                     lbl: 'مواقع',             color: 'var(--info)' },
                  { val: data.totalBreakdowns,                     lbl: 'أيام عطل',          color: 'var(--danger)' },
                ].map((k, i) => (
                  <div key={i} style={{ background: 'var(--steel-3)', border: '1px solid var(--border)', borderRadius: 10, padding: '14px', textAlign: 'center' }}>
                  ...
                  <tbody>
                    {data.eqList.map((e, i) => (
                      <tr key={i}>
                        ...
                        <td>{e.breakdownDays > 0 ? <span className="badge badge-red">{e.breakdownDays} يوم</span> : <span style={{ color: 'var(--text-3)' }}>—</span>}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <div className="print-page print-only">
          ...
          <div className="kpi-grid">
            <div className="kpi-box">
              <div className="kpi-val">{data.totalCost.toLocaleString('ar-SA', { maximumFractionDigits: 0 })}</div>
              <div className="kpi-lbl">💰 إجمالي التكلفة (ريال)</div>
            </div>
            <div className="kpi-box">
              <div className="kpi-val">{data.totalHours.toFixed(0)}</div>
              <div className="kpi-lbl">⏱️ إجمالي الساعات</div>
            </div>
            <div className="kpi-box">
              <div className="kpi-val">{data.eqList.length}</div>
              <div className="kpi-lbl">🏗️ معدات شغلت</div>
            </div>
