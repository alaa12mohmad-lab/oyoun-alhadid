...
export default function UsersPage() {
  ...
  async function loadAll() {
    ...
    setUsers(usrSnap.docs.map(d => ({ id: d.id, ...d.data(), siteName: siteMap[d.data().siteId] || '—' })))
    setSites(siteList)
    setLoading(false)
  }
  ...
  async function save() {
    ...
    try {
      ...
      setErr(e.code === 'auth/email-already-in-use' ? 'البريد الإلكتروني مستخدم مسبقاً' : 'خطأ في إنشاء المستخدم: ' + e.message)
    } finally {
      setSaving(false)
    }
  }
  ...
  async function remove(id) {
    if (!confirm('حذف هذا المستخدم من قاعدة البيانات؟ (لن يُحذف من Firebase Auth تلقائياً)')) return
    await deleteDoc(doc(db, 'users', id))
    loadAll()
  }

  const roleLabel = { admin: { text: 'مدير', cls: 'badge-gold' }, supervisor: { text: 'مشرف', cls: 'badge-blue' }, viewer: { text: 'مشاهد', cls: 'badge-gray' } }
              ...
              <tbody>
                {users.map(u => (
                  <tr key={u.id}>
                    ...
                    <td>{u.role === 'supervisor' ? <span className="badge badge-blue">{u.siteName}</span> : <span style={{ color: 'var(--text-3)' }}>—</span>}</td>
                    <td>
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setModal(false)}>
          <div className="modal">
            ...
            <div className="modal-body">
              ...
              <div className="form-row">
                <div className="form-group">
                  ...
                  <input className="form-control" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="الاسم الكامل" />
                </div>
                <div className="form-group">
                  <label className="form-label">الصلاحية *</label>
                  <select className="form-control" value={form.role} onChange={e => setForm(f => ({ ...f, role: e.target.value, siteId: '' }))}>
              ...
              <div className="form-group">
                ...
                <input type="email" className="form-control" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} placeholder="email@company.com" style={{ direction: 'ltr' }} />
              </div>
              <div className="form-group">
                <label className="form-label">كلمة المرور *</label>
                <input type="password" className="form-control" value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} placeholder="6 أحرف على الأقل" />
              </div>
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-primary" onClick={save} disabled={saving}>{saving ? 'جاري الإنشاء...' : 'إنشاء الحساب'}</button>
            </div>
          </div>
        </div>
      )}
