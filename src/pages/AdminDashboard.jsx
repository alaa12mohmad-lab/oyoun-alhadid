...
import { format, startOfMonth, endOfMonth, differenceInDays, parseISO, eachDayOfInterval, isAfter, isBefore } from 'date-fns'
...
// Days equipment was supposed to work (from startDate to today, within month)
...
export default function AdminDashboard({ setActivePage }) {
  ...
  async function loadAll() {
    try {
      ...
      const siteMap = {}; sites.forEach(s => siteMap[s.id] = s)

      // ── Overall stats ────────────────────────────────────────────
      const totalHours    = logs.reduce((s, l) => s + (l.hours || 0), 0)
      const totalCost     = logs.reduce((s, l) => s + (l.hours || 0) * (l.hourlyRate || eqMap[l.equipmentId]?.hourlyRate || 0), 0)
      ...
      setStats({ totalHours, totalCost, activeEq, breakdowns: breakdownLogs.length, maintenance: maintenanceLogs.length, totalLogs: logs.length })
      ...
      logs.forEach(l => {
        if (!eqSummary[l.equipmentId]) {
          ...
          eqSummary[l.equipmentId] = {
            ...
            hourlyRate: l.hourlyRate || eq?.hourlyRate || 0,
            hours: 0, cost: 0,
        ...
        const cost = (l.hours || 0) * (l.hourlyRate || eqMap[l.equipmentId]?.hourlyRate || 0)
        s.hours += l.hours || 0
        s.cost  += cost
      ...
      // ── Breakdowns ranking ────────────────────────────────────────
      const bdList = eqList
        .filter(e => e.breakdownDays > 0)
        .sort((a, b) => b.breakdownDays - a.breakdownDays)
        .slice(0, 6)
      ...
      // ── Recent logs ───────────────────────────────────────────────
      setRecentLogs(logs.slice(0, 8).map(l => ({
        ...l,
        cost: (l.hours || 0) * (l.hourlyRate || eqMap[l.equipmentId]?.hourlyRate || 0)
      })))
      ...
      setTopWorkers(Object.values(workerMap).sort((a, b) => b.hours - a.hours).slice(0, 5))
  ...
  return (
    <div className="page">
      ...
      <div className="stats-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))' }}>
        <div className="stat-card gold">
          ...
          <div className="stat-value" style={{ fontSize: '1.5rem' }}>
            {stats.totalCost?.toLocaleString('ar-SA', { maximumFractionDigits: 0 })}
          </div>
          <div className="stat-label">إجمالي التكلفة (ريال)</div>
        </div>
        <div className="stat-card green">
          <div className="stat-icon">⏱️</div>
          <div className="stat-value">{stats.totalHours?.toFixed(0)}</div>
          <div className="stat-label">إجمالي الساعات</div>
        </div>
        <div className="stat-card blue">
          <div className="stat-icon">🏗️</div>
          <div className="stat-value">{stats.activeEq}</div>
        ...
        <div className="stat-card" style={{ '--c': 'var(--accent)' }}>
          ...
          <style>{`.stat-card:last-child::before { background: var(--accent); }`}</style>
        </div>
      </div>

      {/* ── Row 1: Site bar chart + Breakdowns ── */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>

        {/* Site cost bar chart */}
        <div className="card">
          <div className="card-header">
            <span className="card-title">📍 تكلفة المواقع — {monthName}</span>
          </div>
            ...
            {siteRows.length === 0 ? (
              ...
              <div key={i} style={{ marginBottom: 14 }}>
                ...
                <div style={{ height: 10, background: 'var(--steel-4)', borderRadius: 5, overflow: 'hidden' }}>
                  <div style={{
                    height: '100%',
                    width: `${(site.cost / maxSiteCost) * 100}%`,
          ...
          <div className="card-body" style={{ padding: '12px 20px' }}>
            {breakdowns.length === 0 ? (
              ...
              <div key={i} style={{
                ...
                borderBottom: i < breakdowns.length - 1 ? '1px solid var(--border)' : 'none'
              }}>
                <div style={{
                  width: 28, height: 28, borderRadius: '50%',
                  background: i === 0 ? '#e05050' : i === 1 ? '#e07030' : 'var(--steel-4)',
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-3)' }}>
                    {eq.siteName}
                    {eq.breakdownReasons.length > 0 && ` · ${eq.breakdownReasons[0]}`}
                  </div>
                </div>
        ...
        <div className="card-body">
          {eqRows.length === 0 ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 12 }}>
              {eqRows.map((eq, i) => (
                <div key={i} style={{
                  background: 'var(--steel-3)', borderRadius: 8,
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                    ...
                    <div style={{ textAlign: 'left' }}>
                      <div style={{
                        ...
                        color: eq.uptime >= 80 ? 'var(--success)' : eq.uptime >= 50 ? 'var(--accent)' : 'var(--danger)'
                      }}>
                        {eq.uptime}%
                      </div>
                  ...
                  <div style={{ height: 6, background: 'var(--steel-4)', borderRadius: 3, overflow: 'hidden', marginBottom: 8 }}>
                    <div style={{
                      ...
                      width: `${eq.uptime}%`,
                      background: eq.uptime >= 80 ? 'var(--success)' : eq.uptime >= 50 ? 'var(--accent)' : 'var(--danger)',
                      transition: 'width 0.6s ease',
                    }} />
                  </div>
      ...
      {/* ── Row 3: Recent logs + Top supervisors ── */}
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16 }}>
          ...
          <div className="table-wrap">
            {recentLogs.length === 0 ? (
              ...
              <table>
                <thead>
                  <tr>
                    <th>التاريخ</th><th>المعدة</th><th>الموقع</th><th>الحالة</th><th>الساعات</th><th>التكلفة</th>
                  </tr>
                </thead>
                <tbody>
                  {recentLogs.map(log => (
        ...
        <div className="card">
          <div className="card-header">
            <span className="card-title">👷 أنشط المشرفين</span>
          </div>
          <div className="card-body" style={{ padding: '12px 20px' }}>
            {topWorkers.length === 0 ? (
              <div className="empty-state"><div className="empty-icon">👷</div><div className="empty-text">لا توجد بيانات</div></div>
              ...
              <div key={i} style={{
                ...
                borderBottom: i < topWorkers.length - 1 ? '1px solid var(--border)' : 'none',
              }}>
                <div style={{
